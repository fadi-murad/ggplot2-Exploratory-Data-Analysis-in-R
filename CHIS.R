...
ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) + 
  geom_histogram(position="fill",aes(y = ..density..), binwidth = 1) +
  BMI_fill
...
ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) + 
  geom_histogram(position="fill",aes(y = ..count../sum(..count..)), binwidth = 1) +
  BMI_fill
...
DF <- table(adult$RBMI,adult$SRAGE_P,dnn=c("RBMI","SRAGE_P"))
